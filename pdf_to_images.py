...
class PdfToImages(object):
        ...
        for d in windirs:
            ...
            for bindir in listing:
                ...
                if len(gswin) == 0:
                    continue
                gs = os.path.abspath(
                    gswin[0])  # Just use the first found .exe (Do i need to do anything more complicated here?)
                os.chdir(cwd)
                return gs
    ...
    def _get_dpi(self, pdf_filename):
        ...
        try:
            ...
            self._warn(
                "Could not execute pdfimages to calculate DPI (try installing xpdf or poppler?), so defaulting to %sdpi" % self.output_dpi)
            return
        ...
        if (results[2] != 'image'):
            self._warn(
                "Could not understand output of pdfimages, please rerun with -d option and file an issue at http://github.com/virantha/pypdfocr/issues")
            return
        ...
        try:
            ...
            logging.debug(str(e))
            self._warn(
                "Could not execute identify to calculate DPI (try installing imagemagick?), so defaulting to %sdpi" % self.output_dpi)
        return
    ...
    @staticmethod
    def insert_subdirectory(for_subdirectory, subdirectory):
        return "/".join((for_subdirectory.split("/")[:-1] + [subdirectory, for_subdirectory.split("/")[-1]]))

    def make_img_from_pdf(self, pdf_filename, subdirectory=""):
        ...
        if subdirectory:
            globable_filename = self.insert_subdirectory(globable_filename, subdirectory)
        ...
        output_filename = '%s_%%04d.%s' % (filename, self.img_file_ext)
        if subdirectory:
            output_filename = self.insert_subdirectory(output_filename, subdirectory)

        self._run_gs(options, output_filename, pdf_filename)
        for fn in glob.glob(globable_filename):
            logging.info("Created image %s" % fn)

        print "Finished! Greyscale? %s" % self.greyscale
